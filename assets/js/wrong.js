...
function renderWrongApp() {
  ...
  if (wrongs.length === 0) {
    el.innerHTML = `
      <div class="card" style="text-align:center;padding:48px">
        ...
        <div style="font-size:.9em;color:#8B8FA3;margin-top:4px">去練題吧！答錯的題目會自動收錄在這裡</div>
  ...
  for (const [subj, items] of Object.entries(grouped)) {
    const color = subjectColors[subj] || '#8B8FA3';
      ...
      html += `
        <div class="wrong-item">
          <div style="display:flex;gap:6px;align-items:center;margin-bottom:6px;flex-wrap:wrap">
            <span class="badge" style="background:rgba(187,173,216,.15);color:#8981C2">${w.year}年 第${w.num}題</span>
            <span style="font-size:.75em;color:#ADA9BE">${w.date}</span>
          ...
          <div style="margin-top:6px;font-size:.8em">
            <span style="color:#C9708E">你的答案：${w.userAnswer || '?'}</span>
            <span style="color:#6E8F7D;margin-left:12px">正確答案：${w.answer || '?'}</span>
          </div>
